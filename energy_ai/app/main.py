...
async def _refresh_pv_forecast():
    ...
    return {"ok": True, "generated_at": forecast["generated_at"], "intervals": len(forecast["rows"]), "interval_minutes": forecast["interval_minutes"], "horizon_hours": forecast["horizon_hours"], "capacity_kw": forecast["capacity_kw"], "radiation_feature": forecast["radiation_feature"], "orientation_configured": forecast["orientation_configured"], "model": forecast["model"], "calibrated_model_active": forecast.get("calibrated_model_active", False)}


async def _refresh_load_forecast():
    forecast = await asyncio.to_thread(load_forecaster.refresh)
    inserted = await asyncio.to_thread(insert_load_forecast, forecast)
    return {"ok": True, "generated_at": forecast["generated_at"], "intervals": inserted, "interval_minutes": forecast["interval_minutes"], "horizon_hours": forecast["horizon_hours"], "model": forecast["model"], "composition": forecast.get("composition"), "flexible_loads": forecast.get("flexible_loads"), "energy_kwh": forecast.get("energy_kwh")}


def _seconds_to_next_quarter() -> float:
    now = datetime.now(timezone.utc); next_minute = ((now.minute // 15) + 1) * 15
    target = now.replace(minute=0, second=20, microsecond=0) + timedelta(hours=1) if next_minute >= 60 else now.replace(minute=next_minute, second=20, microsecond=0)
...
async def lifespan(app):
    ...
    init_db()
    try:
        await collector.run_once(); rebuild_recent_15m(collector.poll_seconds, lookback_hours=48)
    except Exception as exc:
        collector.last_error = repr(exc)
    try: await _refresh_price_horizon()
    except Exception: pass
    try: await _refresh_pv_forecast()
    except Exception: pass
    try: await _refresh_load_forecast()
    except Exception: pass
    try: await asyncio.to_thread(evaluate_matured_forecasts, 7)
    except Exception: pass
    try: await asyncio.to_thread(evaluate_matured_load_forecasts, 7)
    except Exception: pass
    collector_task = asyncio.create_task(collector.loop())
    maintenance_task = asyncio.create_task(_forecast_maintenance_loop())
    yield
    collector.stop()
...
app = FastAPI(title="Energy AI",version=RUNTIME_VERSION,description="Read-only HA energy data, PV/load forecasts, continuous evaluation, flexible loads, training data and LLM analysis",lifespan=lifespan,docs_url=None,redoc_url=None)
...
def _discover_candidates(states: list[dict]) -> dict[str,list[dict]]:
    ...
    for category in ["pv_power","house_load","grid_power","battery_power","battery_soc","spot_price"]:
        ranked=[_row(entity,_candidate_score(entity,category)) for entity in states if _candidate_score(entity,category)>=4]; ranked.sort(key=lambda x:(-x["score"],x["entity_id"] or "")); result[category]=ranked[:30]
    return result
...
async def root():
    return f'''<!doctype html><html lang="sv"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Energy AI</title><style>body{{font-family:system-ui,sans-serif;margin:2rem;max-width:760px}}.ok{{color:#188038}}.warn{{color:#b06000}}code{{background:#8882;padding:.15rem .3rem;border-radius:4px}}li{{margin:.45rem 0}}</style></head><body><h1>Energy AI</h1><p>Home Assistant-appen körs.</p><p id="health">Kontrollerar status…</p><ul><li><a href="training">Training data</a></li><li><a href="forecast/pv/refresh">Refresh PV forecast</a></li><li><a href="forecast/pv">PV forecast</a></li><li><a href="forecast/pv/evaluation">PV online evaluation</a></li><li><a href="forecast/load/refresh">Refresh load forecast</a></li><li><a href="forecast/load">Load forecast</a></li><li><a href="forecast/load/evaluation">Load online evaluation</a></li><li><a href="flexible-loads">EV / sauna state</a></li><li><a href="discover/flexible">Discover Zaptec / sauna entities</a></li><li><a href="prices/refresh">Refresh 15-minute prices</a></li><li><a href="prices">15-minute prices</a></li><li><a href="ha-diagnostics">HA connection diagnostics</a></li><li><a href="discover">Discover HA entities</a></li><li><a href="health">Health</a></li><li><a href="state">Current state</a></li><li><a href="history?resolution=15m&limit=96">15-minute history</a></li><li><a href="history?resolution=raw&limit=100">Raw history</a></li><li><a href="config">Configuration</a></li><li><a href="docs">API docs</a></li></ul><p>Runtime <code>{RUNTIME_VERSION}</code>. Fysisk styrning är avstängd.</p><script>fetch('health').then(r=>r.json()).then(h=>{{const e=document.getElementById('health');e.className=h.ok?'ok':'warn';e.textContent=h.ok?'HA-datainsamling fungerar.':'Appen körs, men datainsamlingen behöver konfigureras: '+(h.last_error||'okänt fel')}})</script></body></html>'''
...
async def forecast_load_refresh():
    try: return await _refresh_load_forecast()
    except Exception as exc: raise HTTPException(500,f"Load forecast refresh failed: {exc!r}")
...
async def forecast_load(limit:int=Query(144,ge=1,le=500)):
    ...
    if result.get("generated_at") is None:
        try:
            await _refresh_load_forecast(); result=latest_load_forecast(limit)
...
async def flexible_loads_state(): return {"ev":ev_state(cfg),"sauna":sauna_state(cfg),"read_only":True}

@app.get("/discover/flexible")
async def discover_flexible():
    try: ranked=await discover_flexible_load_entities(collector.ha)
    except Exception as exc: raise HTTPException(502,f"Could not discover flexible-load entities: {exc!r}")
...
@app.get("/health")
async def health(): return {"ok":collector.last_error is None,"runtime_build":RUNTIME_VERSION,"read_only":True,"collector_running":collector.running,"forecast_evaluation_running":maintenance_task is not None and not maintenance_task.done(),"ha_api_authenticated":collector.ha.authenticated,"last_error":collector.last_error,"openai_configured":explainer.client is not None,"llm_model":explainer.model}
