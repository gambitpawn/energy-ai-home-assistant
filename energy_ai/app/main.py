...
def _discover_candidates(states: list[dict]) -> dict[str, list[dict]]:
    ...
    for category in ["pv_power", "house_load", "grid_power", "battery_power", "battery_soc", "spot_price"]:
        ...
        for entity in states:
            ...
            if score < 4:
                continue
            ranked.append(_row(entity, score))
        ranked.sort(key=lambda x: (-x["score"], x["entity_id"] or ""))
        result[category] = ranked[:30]
    return result
...
def _generic_energy_candidates(states: list[dict]) -> dict[str, list[dict]]:
    ...
    price = []
    keyword = []
    search_words = ["solinteg", "inverter", "battery", "batter", "grid", "meter", "solar", "pv", "power", "nordpool", "tibber", "price"]
    ...
    for entity in states:
        ...
        if device_class == "power" or unit in {"w", "kw"}:
            power.append(row)
        if unit == "%" or device_class == "battery":
            percent.append(row)
        if any(x in unit for x in ["kwh", "mwh", "sek", "kr", "öre", "eur"]):
            if any(x in text for x in ["price", "spot", "nordpool", "tibber", "elpris"]):
                price.append(row)
    ...
    return {
        "all_power_sensors": sorted(power, key=key)[:100],
        "battery_percent_candidates": sorted(percent, key=key)[:100],
        "price_candidates": sorted(price, key=key)[:100],
        "energy_keyword_matches": sorted(keyword, key=key)[:150],
    }


def _table(rows: list[dict], show_score: bool = True) -> str:
    header = "<th>Score</th>" if show_score else ""
    body = []
    for row in rows:
        score = f"<td>{row.get('score', '')}</td>" if show_score else ""
        body.append(
            "<tr>"
            f"<td><code>{escape(str(row.get('entity_id') or ''))}</code></td>"
            f"<td>{escape(str(row.get('friendly_name') or ''))}</td>"
            f"<td>{escape(str(row.get('state') or ''))}</td>"
            f"<td>{escape(str(row.get('unit') or ''))}</td>"
            f"<td>{escape(str(row.get('device_class') or ''))}</td>"
            f"{score}</tr>"
        )
    if not body:
        colspan = 6 if show_score else 5
        body.append(f"<tr><td colspan='{colspan}'>No candidates</td></tr>")
    return (
        "<table><thead><tr><th>Entity</th><th>Name</th><th>State</th><th>Unit</th><th>Device class</th>"
        + header
        + "</tr></thead><tbody>"
        + "".join(body)
        + "</tbody></table>"
...
<title>Energy AI</title><style>body{font-family:system-ui,sans-serif;margin:2rem;max-width:760px}.ok{color:#188038}.warn{color:#b06000}code{background:#8882;padding:.15rem .3rem;border-radius:4px}li{margin:.45rem 0}</style></head>
<body><h1>Energy AI</h1><p>Home Assistant-appen körs.</p><p id="health">Kontrollerar status…</p><ul>
<li><a href="ha-diagnostics">HA connection diagnostics</a></li><li><a href="discover">Discover HA entities</a></li><li><a href="health">Health</a></li><li><a href="state">Current state</a></li><li><a href="history">History</a></li><li><a href="config">Configuration</a></li><li><a href="docs">API docs</a></li></ul>
<p>Version <code>0.1.7</code>. Fysisk styrning är avstängd.</p><script>fetch('health').then(r=>r.json()).then(h=>{const e=document.getElementById('health');e.className=h.ok?'ok':'warn';e.textContent=h.ok?'HA-datainsamling fungerar.':'Appen körs, men datainsamlingen behöver konfigureras: '+(h.last_error||'okänt fel')}).catch(()=>{document.getElementById('health').textContent='Health-status kunde inte läsas.'})</script></body></html>
"""
...
async def discover():
    ...
    generic = _generic_energy_candidates(states)
    labels = {
        "pv_power": "PV power",
    ...
    sections = [f"<h2>{labels[k]}</h2>{_table(v)}" for k, v in groups.items()]
    sections += [
        "<h2>All power sensors</h2>" + _table(generic["all_power_sensors"], False),
        "<h2>Battery / percentage candidates</h2>" + _table(generic["battery_percent_candidates"], False),
        "<h2>Price candidates</h2>" + _table(generic["price_candidates"], False),
        "<h2>Energy keyword matches</h2>" + _table(generic["energy_keyword_matches"], False),
    ]

    return """
<!doctype html><html lang="sv"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Energy AI discovery</title><style>body{font-family:system-ui,sans-serif;margin:2rem;max-width:1200px}table{border-collapse:collapse;width:100%;margin-bottom:2rem}th,td{border-bottom:1px solid #9995;text-align:left;padding:.42rem}code{font-size:.88em}h2{margin-top:2.2rem}</style></head><body>
<h1>Discover Home Assistant entities</h1><p>Rankade kandidater följs av bredare listor baserade på enhet, device_class och energirelaterade namn. Ingen kandidat betraktas som verifierad ännu.</p>
...
@app.get("/history")
async def history(resolution: str = Query("15m", pattern="^(raw|15m)$"), limit: int = Query(96, ge=1, le=1000)):
