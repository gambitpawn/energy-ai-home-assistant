...
def horizon_diagnostics(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    known_until = (_parse_ts(last_known) + timedelta(minutes=15)).isoformat() if last_known else None
    return {
...
def _build_horizon(cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not starts:
        raise RuntimeError(
            "No overlapping load and PV forecast rows are available after UTC normalization; "
            f"diagnostics={horizon_diagnostics(cfg)}"
        )
...
def _state_grid(min_kwh: float, max_kwh: float, requested_step_kwh: float) -> tuple[list[float], float]:
    """Create an evenly spaced inclusive SOC grid.

    The configured step is a maximum spacing. The usable range is divided into
    enough equal intervals that both hard boundaries are native grid states.
    """
    span = max_kwh - min_kwh
    if span <= 0:
        return [round(min_kwh, 6)], 0.0
    requested_step_kwh = max(1e-6, requested_step_kwh)
    intervals = max(1, int(math.ceil(span / requested_step_kwh)))
    effective_step = span / intervals
    states = [round(min_kwh + i * effective_step, 6) for i in range(intervals + 1)]
    states[0] = round(min_kwh, 6)
    states[-1] = round(max_kwh, 6)
    return states, effective_step
...
def _continuation_profile(
    rows: list[dict[str, Any]], cfg: dict[str, Any], capacity: float,
    preferred_max_kwh: float, eta_discharge: float,
) -> dict[str, Any]:
    opt = cfg.get("optimizer") or {}
    ...
    coverage_fraction = max(0.0, min(1.0, float(opt.get("unknown_price_energy_coverage_fraction", 0.35))))
    risk_premium_max = max(0.0, float(opt.get("unknown_price_risk_premium_ore_kwh", 40.0)))
    default_value = max(0.0, float(opt.get("unknown_price_default_continuation_value_ore_kwh", 150.0)))
    full_scale_kw = max(0.01, float(opt.get("reserve_uncertainty_full_scale_kw", 3.0)))
    ...
    net_deficit_kwh = sum(max(0.0, float(r["load_kw"]) - float(r["pv_kw"])) * DT_HOURS for r in unknown)
    peak_support_kwh = sum(
        max(0.0, float(r["load_kw"]) - float(r["pv_kw"]) - import_limit)
        * DT_HOURS / max(0.01, eta_discharge) for r in unknown
    )
    covered_energy_kwh = net_deficit_kwh * coverage_fraction / max(0.01, eta_discharge)
    target_kwh = min(
        preferred_max_kwh,
        max(reserve_target_kwh + covered_energy_kwh, reserve_target_kwh + peak_support_kwh),
    )
    known_buy_prices = [
    ...
    avg_uncertainty = sum(
        ...
        + max(0.0, float(r.get("pv_uncertainty_kw") or 0.0)) for r in unknown
    ) / len(unknown)
    ...
    return {
        ...
        "value_ore_per_kwh": reference + risk_premium,
...
def _interval_result(row: dict[str, Any], action_kw: float, cfg: dict[str, Any]) -> dict[str, float]:
    ...
    discretionary_discharge_kw = max(0.0, action_kw - required_physical_discharge_kw) if action_kw > 0.0 else 0.0
    ...
    if not price_known:
        ...
        degradation_cost = abs(action_kw) * DT_HOURS * degradation
        discretionary_shift_hurdle_cost = discretionary_discharge_kw * DT_HOURS * minimum_shift_margin
...
def _classify_action(row: dict[str, Any], action_kw: float, result: dict[str, float]) -> tuple[str, dict[str, float]]:
    if not row.get("price_known"):
        if action_kw > 0.05:
            return "physical_limit_discharge", {"battery_to_load_kw": action_kw, "battery_to_grid_kw": 0.0}
        if action_kw < -0.05:
            return "pv_charge_unpriced", {"pv_to_battery_kw": result.get("pv_charge_kw", 0.0), "grid_to_battery_kw": 0.0}
    ...
    if action_kw < -0.05:
        ...
        reason = "mixed_charge" if pv_charge_kw > 0.05 and grid_charge_kw > 0.05 else "pv_charge" if pv_charge_kw > 0.05 else "grid_charge"
...
def build_plan(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    requested_state_step = float(opt.get("soc_grid_step_kwh", 0.5))
    ...
    states, effective_state_step = _state_grid(min_kwh, max_kwh, requested_state_step)
    continuation = _continuation_profile(rows, cfg, capacity, preferred_max_kwh, eta_discharge)
    known_count = sum(1 for r in rows if r.get("price_known"))
    boundary_idx = known_count - 1 if 0 < known_count < len(rows) else None

    # The measured initial SOC is exact for the first transition only; it is not
    # inserted as a permanent off-grid state that could create later short hops.
    costs: dict[int, float] = {}
    parents: list[dict[int, tuple[int, float, dict[str, float], float, float, float]]] = []
    for t, row in enumerate(rows):
        ...
        source_states = [(-1, initial_kwh, 0.0)] if t == 0 else [
            (i0, states[i0], prior_cost) for i0, prior_cost in costs.items()
        ]
        for i0, e0, prior_cost in source_states:
            for i1, e1 in enumerate(states):
                ...
                if e1 < preferred_min_kwh:
                    policy_adjustment += (preferred_min_kwh - e1) * reserve_penalty * 0.10
                if e1 > preferred_max_kwh:
                    policy_adjustment += (e1 - preferred_max_kwh) * reserve_penalty * 0.02
                if boundary_idx is not None and t == boundary_idx:
                    target_kwh = float(continuation.get("target_kwh") or 0.0)
                    reference_value = float(continuation.get("reference_price_ore_kwh") or 0.0)
                    risk_premium = float(continuation.get("risk_premium_ore_kwh") or 0.0)
                ...
                if i1 not in next_costs or total < next_costs[i1]:
                    next_costs[i1] = total
                    back[i1] = (i0, action, result, reserve_kwh, reserve_pct, policy_adjustment)
        if not next_costs:
    ...
    for t in range(len(rows) - 1, -1, -1):
        prev_idx, action, result, reserve_kwh, reserve_pct, policy_adjustment = parents[t][idx]
        path.append((prev_idx, idx, action, result, reserve_kwh, reserve_pct, policy_adjustment))
        idx = prev_idx
    path.reverse()

    out_rows = []
    objective_cost = cash_cost = total_battery_export_kwh = 0.0
    total_discretionary_discharge_kwh = total_shift_hurdle_ore = 0.0
    total_policy_adjustment_ore = 0.0
    boundary_soc_pct = None
    for t, (row, (_, i1, action, result, reserve_kwh, reserve_pct, policy_adjustment)) in enumerate(zip(rows, path)):
        ...
        total_discretionary_discharge_kwh += result.get("discretionary_discharge_kw", 0.0) * DT_HOURS
        total_shift_hurdle_ore += result.get("discretionary_shift_hurdle_cost_ore", 0.0)
        ...
        out_rows.append(
            {
                **row,
                ...
                "required_physical_discharge_kw": round(result.get("required_physical_discharge_kw", 0.0), 4),
                "discretionary_discharge_kw": round(result.get("discretionary_discharge_kw", 0.0), 4),
                "curtailed_kw": round(result["curtailed_kw"], 4),
                "energy_cost_ore": round(result["energy_cost_ore"], 4) if row.get("price_known") else None,
                "degradation_cost_ore": round(result["degradation_cost_ore"], 4),
                "cash_cost_ore": round(result["cash_cost_ore"], 4),
                "discretionary_shift_hurdle_cost_ore": round(result.get("discretionary_shift_hurdle_cost_ore", 0.0), 4),
                "arbitrage_hurdle_cost_ore": round(result.get("discretionary_shift_hurdle_cost_ore", 0.0), 4),
                "policy_adjustment_ore": round(policy_adjustment, 4),
                "objective_cost_ore": round(result["interval_cost_ore"] + policy_adjustment, 4),
                "reason": reason,
    ...
    continuation_reference = float(continuation.get("reference_price_ore_kwh") or 0.0)
    continuation_risk = float(continuation.get("risk_premium_ore_kwh") or 0.0)
    continuation_target_kwh = float(continuation.get("target_kwh") or 0.0)
    boundary_kwh = capacity * boundary_soc_pct / 100.0 if boundary_soc_pct is not None else states[best_idx]
    optimized_continuation_asset_ore = baseline_continuation_asset_ore = 0.0
    if continuation.get("enabled"):
    ...
    economic_saving_ore = cash_saving_ore + optimized_continuation_asset_ore - baseline_continuation_asset_ore
    return {
        ...
        "constraints": {
            ...
            "physical_grid_import_limit_kw": float(opt.get("physical_grid_import_limit_kw", 13.8)),
            ...
            "soc_grid_requested_step_kwh": requested_state_step,
            "soc_grid_effective_step_kwh": round(effective_state_step, 6),
            "soc_grid_state_count": len(states),
            "soc_grid_includes_hard_boundaries": True,
            "initial_soc_exact_first_transition": True,
        ...
        "objective": {
            ...
            "minimum_net_discretionary_shift_margin_ore_kwh": float(
                ((cfg.get("policy") or {}).get("economics") or {}).get("minimum_arbitrage_margin_ore_kwh", 20.0)
            ),
        ...
        "continuation": {
            ...
            "price_boundary_soc_pct": round(boundary_soc_pct, 2) if boundary_soc_pct is not None else None,
        ...
        "summary": {
            ...
            "optimized_continuation_asset_value_ore": round(optimized_continuation_asset_ore, 2),
            "baseline_continuation_asset_value_ore": round(baseline_continuation_asset_ore, 2),
            ...
            "discretionary_discharge_kwh": round(total_discretionary_discharge_kwh, 3),
            "discretionary_shift_hurdle_cost_ore": round(total_shift_hurdle_ore, 2),
