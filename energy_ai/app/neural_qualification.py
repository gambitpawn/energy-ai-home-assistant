...
from . import hybrid_engine, neural_engine
from .neural_features import FEATURE_SCHEMA
from . import neural_training

CANDIDATE_STATE_PATH = Path("/data/models/neural_v1_qualification.json")
POLICY_ID = "frozen_candidate_robust10_v1"
LEARNED_ENGINE_IDS = {"neural_v1", "hybrid_v1"}
...
def _latest_meta() -> dict[str, Any]:
    return _read_json(neural_training.MODEL_META_PATH)
...
def _candidate_source_valid(candidate: dict[str, Any]) -> bool:
    ...
    if not model_id:
        return False
    if candidate.get("feature_schema") != FEATURE_SCHEMA:
        return False
    if not bool(candidate.get("shadow_ready")):
        return False
    return _version_model_path(str(model_id)).exists()


def _snapshot_latest(*, reason: str, previous: dict[str, Any] | None = None, details: dict[str, Any] | None = None) -> dict[str, Any]:
    latest = _latest_meta()
    if not latest:
    ...
    generation = int(prior.get("qualification_generation") or 0) + 1
    candidate = {
        **latest,
...
def ensure_qualification_candidate() -> dict[str, Any]:
    ...
    return _snapshot_latest(reason=reason, previous=candidate or None)


def load_qualification_model() -> tuple[Any, dict[str, Any]]:
    ensured = ensure_qualification_candidate()
    if not ensured.get("ok"):
        raise FileNotFoundError(f"no usable neural qualification candidate: {ensured.get('status')}")
    meta = dict(ensured["candidate"])
    model_id = str(meta["model_id"])
    model = joblib.load(_version_model_path(model_id))
    return model, meta
...
def qualification_status() -> dict[str, Any]:
    ...
    return {
        ...
        "rotation_policy": "after completed failed robust10 qualification, never while neural_v1 or hybrid_v1 is incumbent",
    }


def install_qualification_candidate_runtime() -> dict[str, Any]:
    """Route neural/hybrid race decisions to the frozen candidate artifact.

    Automatic training continues to update neural_training.MODEL_PATH and metadata.
    Only EngineDecision production is patched; training data/model maintenance is untouched.
    """
    neural_engine.load_model = load_qualification_model
    hybrid_engine.load_model = load_qualification_model
