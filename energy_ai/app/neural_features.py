...
FEATURE_SCHEMA = "neural_v1_features_v3"
# Multi-resolution horizon: preserve the exact 15-minute structure close to the
# decision, then compress progressively farther into the future.
# (prefix, block_count, 15-minute intervals per block)
HORIZON_BLOCK_LAYOUT = (
    ("q", 24, 1),   # 0-6 h: 15-minute blocks
    ("h", 12, 4),   # 6-18 h: 1-hour blocks
    ("b", 9, 8),    # 18-36 h: 2-hour blocks
)
MAX_HORIZON_INTERVALS = sum(count * intervals for _, count, intervals in HORIZON_BLOCK_LAYOUT)
BLOCK_FEATURES = (
...
def feature_names():
    names = [
        "initial_soc_pct", "decision_hour_sin", "decision_hour_cos", "decision_dow_sin", "decision_dow_cos",
        "horizon_fraction", "price_known_fraction", "known_price_min_ore_kwh", "known_price_max_ore_kwh", "known_price_spread_ore_kwh",
        "forecast_load_energy_kwh", "forecast_pv_energy_kwh", "forecast_net_energy_kwh", "mean_load_uncertainty_kw", "mean_pv_uncertainty_kw",
        *SYSTEM_FEATURES,
    ]
    for prefix, count, _intervals in HORIZON_BLOCK_LAYOUT:
        for block in range(count):
            for name in BLOCK_FEATURES:
                names.append(f"{prefix}{block:02d}_{name}")
    return names
...
def _append_block_features(out, chunk, consumption, production, ce, pe):
    if not chunk:
        out.extend([0.0] * len(BLOCK_FEATURES))
        return
    bl = [float(r.get("load_kw") or 0) for r in chunk]
    bp = [float(r.get("pv_kw") or 0) for r in chunk]
    ...
    out.extend([
        _mean(bl), _mean(bp), _mean([l - p for l, p in zip(bl, bp)]), _mean(bu), _mean(bk), len(bk) / float(len(chunk)),
        _tariff_active_fraction(chunk, consumption, ce), _tariff_active_fraction(chunk, production, pe),
    ])


def vectorize(engine_input):
    rows = list(engine_input.horizon_rows)[:MAX_HORIZON_INTERVALS]
    dt_h = float(engine_input.interval_minutes) / 60.0
    decision = datetime.fromisoformat(engine_input.decision_start.replace("Z", "+00:00")).astimezone(LOCAL_TZ)
    hour = decision.hour + decision.minute / 60.0; dow = float(decision.weekday())
    ...
    out = [
        float(engine_input.initial_soc_pct), math.sin(2 * math.pi * hour / 24), math.cos(2 * math.pi * hour / 24),
        math.sin(2 * math.pi * dow / 7), math.cos(2 * math.pi * dow / 7), len(rows) / float(MAX_HORIZON_INTERVALS),
        kf, pmin, pmax, pmax - pmin, sum(loads) * dt_h, sum(pvs) * dt_h,
        sum(l - p for l, p in zip(loads, pvs)) * dt_h, _mean(load_u), _mean(pv_u), *_system_vector(engine_input),
    ]
    tariffs = (engine_input.objective or {}).get("tariffs") or {}; te = bool(tariffs.get("enabled"))
    consumption = tariffs.get("consumption_demand") or {}; production = tariffs.get("production_demand") or {}
    ce = te and bool(consumption.get("enabled")); pe = te and bool(production.get("enabled"))

    offset = 0
    for _prefix, count, intervals_per_block in HORIZON_BLOCK_LAYOUT:
        for _block in range(count):
            chunk = rows[offset:offset + intervals_per_block]
            _append_block_features(out, chunk, consumption, production, ce, pe)
            offset += intervals_per_block
...
def feature_metadata():
    return {
        ...
        "horizon_layout": [
            {"prefix": prefix, "block_count": count, "intervals_per_block": intervals, "block_hours": intervals * .25}
            for prefix, count, intervals in HORIZON_BLOCK_LAYOUT
        ],
        "maximum_horizon_hours": MAX_HORIZON_INTERVALS * .25,
